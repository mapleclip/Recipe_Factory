...
public class FaqboardInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Faqboard fb = new Faqboard(writer,title,content);
		
		int result = new FaqService().insertFboard(fb);
		
		if(result > 0) {
			response.sendRedirect("FAQListServlet.fa");
		} else {
			request.setAttribute("msg", "자주묻는 질문 등록에 실패하였습니다");
